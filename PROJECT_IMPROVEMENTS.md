...
角色:全栈独立开发(设计→实现→测试→部署全流程)
技术栈:Spring Boot 3 / MyBatis-Plus / MySQL / Redis / Spring Security
        · Vue 3 / TypeScript / Vite / Docker / GitHub Actions
```

### 详版(叙事式,推荐)

> 原则:每条都是"场景/问题 → 方案 → 效果",不罗列技术名词。

```
- 业务闭环:独立设计实现图书商城全流程(会员→两级分类检索→购物车→订单状态流转→店主后台),
  覆盖图书/进货/日结/客户管理,从需求到部署全链路负责
- 缓存性能优化:分析热点后对图书详情/列表/分类/公告加 Redis + Spring Cache 缓存,
  写操作 @CacheEvict 自动失效 + TTL 兜底保证一致性;冷热请求实测接口延迟 562ms→13ms,缓存命中率约 95%
- 库存防超卖:下单扣库存采用数据库条件更新 UPDATE ... WHERE stock >= 数量,原子操作保证库存不足即失败回滚,
  规避并发超卖(单机规模用 DB 原子操作即可,分布式才需要引入锁)
- 订单状态机:用 OrderStatus 枚举 + 合法流转表统一校验状态跳转,拦截非法流转;
  并修复了"已汇款(0→1)"状态缺失导致店主无法确认收款的 bug
- 前端工程化:Vue3 + TypeScript 全量类型化,openapi-typescript 由后端 OpenAPI 契约自动生成类型;
  Element Plus 按需引入使主包 1.1MB→326KB;服务端分页避免全量拉取
- 工程化与部署:19 个后端单测(Mockito,覆盖状态机/库存/日结)+ 7 个前端单测(Vitest+MSW),
  GitHub Actions CI(含 MySQL 服务容器)、Docker Compose 一键部署
- 踩坑复盘:定位并修复缓存反序列化失败(顶层 List 类型标注缺失)、登录 403(CORS IPv6 来源 + Vite 端口漂移)
  等问题,能完整还原"现象→定位→修复→回归"的排查过程
```

### 简版(3 条)

```
- 独立完成图书商城全栈:会员、检索、购物车、订单状态流转、店主后台(进货/日结/客户)完整闭环
- 后端 Spring Boot + MyBatis-Plus + Redis + Spring Security;Redis 缓存冷热实测 562ms→13ms(约40倍)、
  命中率95%;DB 条件更新防库存超卖;订单状态机拦截非法流转
- 前端 Vue3 + TypeScript,openapi-typescript 前后端类型同步,按需引入主包降70%;
  19 后端 + 7 前端单测、GitHub Actions CI、Docker Compose 部署
```
...
1. **"命中率 95% 怎么测的?"** → 诚实说:本地压测,清空缓存后冷请求 vs 连续热请求,对比后端 SQL 次数(21 次请求仅 1 次查库)算出。**主动说明局限**:是本地构造的热点访问,不是真实用户流量,真实场景会低一些。
2. **"超卖怎么防的?"** → DB 条件更新 `WHERE stock >= 数量`,原子且单机够用;如果上分布式/高并发,才需要 Redis 分布式锁或乐观锁版本号。**别假装用了 Redis 锁**——没做就是没做。
3. **"缓存一致性?"** → 主动失效(@CacheEvict)+ TTL 兜底;接受短暂不一致,极端场景可上延迟双删。
4. **"日结一致性?"** → @Transactional 保证一次日结要么全成要么回滚;自调用事务失效的坑我踩过。
5. **"配送→完成谁触发?"** → 店主后台手动确认(业务场景是线下配送后店主点确认),不是定时任务/Webhook——坦率说明即可。
6. **"检索是 ES 还是 LIKE?"** → MySQL LIKE,当前规模够用;**主动说**:数据量大后可换 ES/全文索引。
7. **"独立开发是不是没团队?"** → 坦诚单人,但强调:因此我承担了设计/实现/测试/部署全链路,有"全栈设计思维";同时说明多人协作下会怎么分工(契约先行 + Flyway + PR)。

### 写简历的 3 条铁律(修订)

1. **叙事式 > 清单式**——每条写成"问题→方案→效果",别堆技术名词
2. **数字要能解释来源**——95%、40 倍这些一定要能说清怎么测的,说不清宁可不写
3. **没做的别写**——"分布式锁""高并发""微服务"一个都不许出现,写了必翻车
